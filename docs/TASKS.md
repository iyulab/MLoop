...
#### Phase 0 & 1 Core (2024-11 ~ 2025-11)
- [x] .NET 9 솔루션 구조 생성
- [x] Global tool 설정 (PackAsTool, ToolCommandName)
- [x] 핵심 의존성 추가 (ML.NET 4.0.3, AutoML 0.22.3, Spectre.Console)
...
- [x] FileSystemManager 구현
- [x] ProjectDiscovery 구현 (.mloop 디렉토리 탐색)
- [x] ExperimentStore 구현 (exp-XXX ID 생성)
- [x] ConfigMerger 구현 (CLI > mloop.yaml > .mloop/config.json > defaults)

#### Core Layer (완료)
- [x] CsvDataLoader 구현 (ML.NET TextLoader)
- [x] AutoMLRunner 구현
  - Binary Classification (Accuracy, AUC, F1)
  - Multiclass Classification (Micro/Macro Accuracy)
  - Regression (MAE, RMSE, R²)
- [x] TrainingEngine 구현 (실험 메타데이터 저장)

#### CLI Layer (완료)
- [x] InitCommand (mloop init <project> --task <type>)
- [x] TrainCommand (mloop train <data> --label <col>)
- [x] Progress reporting (Spectre.Console)
- [x] Training summary table (metrics 표시)

#### E2E 검증 (완료)
- [x] Binary classification 테스트 (Titanic 스타일)
- [x] Multiclass classification 테스트 (Iris 스타일)
- [x] Regression 테스트 (공급망 데이터셋)
  - Excel (.xls) → CSV 변환
  - Lag features 생성
  - FastForest 모델 학습 (MAE: 16.17, R²: 0.88)

### 현재 상태 (2025-11-03 업데이트)
- **빌드**: ✅ 성공 (mloop 실행 가능)
- **Phase 1 MVP**: ✅ 핵심 기능 완료
  - InitCommand: ✅ 프로젝트 초기화 동작
  - TrainCommand: ✅ Binary/Multiclass/Regression 모두 지원
  - E2E 테스트: ✅ 공급망 데이터셋으로 검증 완료
- **ML.NET 통합**: ✅ 완료 (ML.NET 4.0.3 + AutoML 0.22.3)
- **다음 단계**: AutoML 최적화 및 사용성 개선
...
### 1.6 AutoML 최적화 및 사용성 개선 (Phase 1.5)

**목표**: 큰 지식 없이도 기본 설정으로 최선의 결과를 내도록 개선
**예상 기간**: 2-3주
**우선순위**: 🔴 최고

#### 설계 원칙
- **Zero-config Philosophy**: 최소한의 입력으로 최대한의 성능
- **Smart Defaults**: 데이터 특성 기반 자동 설정
- **Progressive Disclosure**: 필요시에만 고급 옵션 노출
- **Guided Experience**: 명확한 피드백과 권장사항 제공

#### 태스크 목록

##### 1.6.1 데이터 분석 및 자동 설정
**우선순위**: 🔴 P0

**세부 태스크**:
- [ ] **T1.6.1.1**: 데이터 프로파일러 구현
  ```
  src/MLoop/Core/Data/DataProfiler.cs
  ```
  - 컬럼 타입 자동 감지 (numeric, categorical, datetime)
  - 결측치 비율 계산
  - 데이터 분포 분석 (skewness, outliers)
  - 클래스 불균형 감지 (classification)
  - 시계열 패턴 감지 (시간 컬럼 존재 + 순차성)
  - **예상 시간**: 6시간

- [ ] **T1.6.1.2**: 자동 학습 시간 추정
  - 데이터 크기 기반 time limit 자동 설정
    - Small (<1K rows): 60초
    - Medium (1K-10K): 180초
    - Large (10K-100K): 300초
    - XLarge (>100K): 600초
...
- [ ] **T1.6.1.3**: 자동 Metric 선택
  - Binary classification: AUC (불균형시), Accuracy (균형시)
  - Multiclass: Micro-Accuracy (많은 클래스), Macro-Accuracy (적은 클래스)
  - Regression: MAE (outlier 많음), RMSE (정규분포)
  - **예상 시간**: 3시간

- [ ] **T1.6.1.4**: 자동 Test Split 설정
  - 데이터 크기 기반:
    - <1K rows: 30%
    - 1K-10K: 20%
    - >10K: 10%
...
**완료 조건**:
- [x] 데이터 프로파일링 정확함
- [x] 모든 설정이 자동으로 최적값 선택됨
- [x] 사용자는 data-file과 label만 제공

##### 1.6.2 데이터 품질 검증 및 전처리
**우선순위**: 🔴 P0

**세부 태스크**:
- [ ] **T1.6.2.1**: 데이터 품질 검증기
  ```
  src/MLoop/Core/Data/DataValidator.cs
  ```
  - Label 컬럼 존재 확인
  - 최소 데이터 크기 확인 (>50 rows)
  - 고유 Label 값 확인 (classification: >1, regression: continuous)
  - 결측치 과다 경고 (>50%)
  - **예상 시간**: 4시간

- [ ] **T1.6.2.2**: 자동 전처리 파이프라인
  ```
  src/MLoop/Core/Data/AutoPreprocessor.cs
  ```
  - 결측치 처리 (numeric: median, categorical: mode)
  - 범주형 인코딩 (One-hot 또는 Hash)
  - 수치형 정규화 (선택적, 스케일 차이 큰 경우만)
  - 이상치 처리 (선택적, z-score > 3)
  - **예상 시간**: 8시간

- [ ] **T1.6.2.3**: 시계열 데이터 자동 감지 및 처리
  - CRET_TIME, timestamp, date 컬럼 감지
  - 자동 정렬 (시간 순)
  - 시간 피처 추출 옵션 제공 (요일, 시간, 주차)
  - Lag features 자동 제안 (--suggest-lag-features)
  - **예상 시간**: 6시간

**완료 조건**:
- [x] 데이터 품질 문제 자동 감지 및 경고
- [x] 전처리 자동 수행 (사용자 개입 최소화)
- [x] 시계열 데이터 자동 처리
...
- [ ] **T1.6.3.1**: 성능 해석기
  ```
  src/MLoop/Infrastructure/Insights/PerformanceInterpreter.cs
  ```
  - Metric 기반 모델 품질 평가
    - Excellent (>0.9), Good (0.7-0.9), Fair (0.5-0.7), Poor (<0.5)
  - 비즈니스 친화적 설명 제공
  - **예상 시간**: 4시간

- [ ] **T1.6.3.2**: 개선 제안 엔진
  - 데이터 품질 기반 제안
    - "결측치가 많습니다. 데이터 수집 개선을 권장합니다"
    - "클래스 불균형이 심합니다. 데이터 증강을 고려하세요"
    - "데이터가 적습니다. 추가 데이터 수집을 권장합니다"
  - 학습 시간 기반 제안
    - "더 긴 학습 시간으로 성능 향상 가능: --time 600"
  - 피처 엔지니어링 제안
    - "시계열 데이터 감지: lag features 사용 고려"
  - **예상 시간**: 5시간

- [ ] **T1.6.3.3**: Next Steps 가이드
  - 학습 완료 후 명확한 다음 단계 제시
  - 평가 방법 안내
  - 예측 사용법 안내
  - 프로덕션 배포 가이드
  - **예상 시간**: 3시간
...
**세부 태스크**:
- [ ] **T1.6.4.1**: 대화형 초기화 모드
  - `mloop init` (인자 없이) → 대화형 프롬프트
  - 프로젝트명, 작업 유형, 데이터 경로 등 단계별 입력
  - **예상 시간**: 4시간

- [ ] **T1.6.4.2**: Smart Train 커맨드
  - `mloop train <data>` (label 자동 감지 시도)
    - 마지막 컬럼이 label인지 확인 프롬프트
  - `mloop train <data> --auto`
    - 모든 설정 자동 (time, metric, test-split)
  - **예상 시간**: 3시간

- [ ] **T1.6.4.3**: 학습 중 실시간 피드백 개선
  - 현재 best metric 표시
  - 남은 시간 추정
  - 시도한 알고리즘 목록 (Live Table)
  - **예상 시간**: 4시간

- [ ] **T1.6.4.4**: 결과 요약 개선
  - 비교 테이블 (baseline vs best model)
  - 피처 중요도 표시 (가능한 경우)
  - 모델 복잡도 표시
  - **예상 시간**: 4시간
...
**완료 조건**:
- [x] 초보자도 쉽게 사용 가능
- [x] 학습 과정이 투명하고 이해하기 쉬움
- [x] 결과가 명확하고 실행 가능

##### 1.6.5 Configuration Presets
**우선순위**: 🟢 P2

**세부 태스크**:
- [ ] **T1.6.5.1**: 사전 정의된 프로파일
  ```yaml
  # Fast mode (프로토타이핑)
  time_limit: 60
  optimize_for: speed
...
- [ ] **T1.6.5.2**: 도메인별 템플릿
  ...
  - Time series forecasting preset
  - Image classification preset (향후)
  - **예상 시간**: 4시간

**완료 조건**:
- [x] `mloop train data.csv --preset fast` 동작
- [x] 도메인별 최적 설정 제공
...
#### Acceptance Criteria
- [x] **AC1**: `mloop train data.csv` 만으로 학습 가능 (label 자동 감지)
- [x] **AC2**: 데이터 크기에 따른 자동 설정 (time, metric, split)
- [x] **AC3**: 데이터 품질 자동 검증 및 전처리
- [x] **AC4**: 시계열 데이터 자동 감지 및 처리
- [x] **AC5**: 학습 결과 해석 및 개선 제안 제공
- [x] **AC6**: 비기술자도 이해 가능한 출력

#### Definition of Done
- [x] 모든 Phase 1.5 태스크 완료
- [x] Zero-config 워크플로우 검증
- [x] 사용자 테스트 (비 ML 전문가)
- [x] 문서 업데이트 (Getting Started 가이드)
...
### 전체 진행률 (2025-11-03 업데이트)

| Phase | 진행률 | 상태 | 완료일 |
|-------|--------|------|--------|
| Phase 0 | 100% | ✅ 완료 | 2024-11 |
| Phase 1 | 85% | 🟢 대부분 완료 | 2025-11 |
| Phase 1.5 | 0% | 📋 계획됨 | - |
| Phase 2 | 0% | 🔄 대기 중 | - |
| Phase 3 | 0% | 🔄 대기 중 | - |
| Phase 4 | 0% | 🔄 대기 중 | - |
...
| 컴포넌트 | 태스크 수 | 완료 | 진행률 | 상태 |
|----------|-----------|------|--------|------|
| Infrastructure | 14 | 14 | 100% | ✅ 완료 |
| Core | 17 | 15 | 88% | 🟢 거의 완료 |
| CLI | 16 | 13 | 81% | 🟢 거의 완료 |
| Testing | 9 | 6 | 67% | 🟡 진행 중 |
| Documentation | 3 | 1 | 33% | 🟡 부분 완료 |
...
| 컴포넌트 | 태스크 수 | 완료 | 진행률 | 우선순위 |
|----------|-----------|------|--------|----------|
| 데이터 분석 및 자동 설정 | 4 | 0 | 0% | 🔴 P0 |
| 데이터 품질 검증 및 전처리 | 3 | 0 | 0% | 🔴 P0 |
| 결과 해석 및 권장사항 | 3 | 0 | 0% | 🟡 P1 |
| CLI UX 개선 | 4 | 0 | 0% | 🟡 P1 |
| Configuration Presets | 2 | 0 | 0% | 🟢 P2 |
