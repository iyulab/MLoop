...
public class PreprocessingScriptGeneratorTests : IDisposable
{
    ...
    public void GenerateScript_EmptyIssues_GeneratesValidStructure()
    {
        ...
        // Must match the authoritative IPreprocessingScript contract:
        // Task<string> ExecuteAsync(PreprocessContext context) — NOT the non-existent
        // PreprocessingResult/PreprocessingContext types that previously caused generated
        // scripts to fail compilation. See ScriptGeneratesCompilableScript test below.
        Assert.Contains("Task<string> ExecuteAsync(PreprocessContext context)", script);
        Assert.DoesNotContain("PreprocessingResult", script);
        Assert.DoesNotContain("PreprocessingContext", script);
        Assert.DoesNotContain("GetTempPath", script);
    }
    ...
    #region GenerateScript - Compilation (regression: generated code must match real contract)

    public static IEnumerable<object[]> IssueCombinations()
    {
        yield return new object[] { Array.Empty<DataQualityIssueType>() };
        yield return new object[] { new[] { DataQualityIssueType.EncodingIssue } };
        yield return new object[] { new[] { DataQualityIssueType.DuplicateRows } };
        yield return new object[] { new[] { DataQualityIssueType.WhitespaceIssues } };
        yield return new object[]
        {
            new[]
            {
                DataQualityIssueType.EncodingIssue,
                DataQualityIssueType.DuplicateRows,
                DataQualityIssueType.WhitespaceIssues
            }
        };
    }

    [Theory]
    [MemberData(nameof(IssueCombinations))]
    public async Task GeneratedScript_CompilesAndLoadsAsIPreprocessingScript(DataQualityIssueType[] types)
    {
        // The generator's output is consumed by ScriptLoader at runtime. Asserting on substrings
        // is not enough — previously the generated code referenced non-existent types and silently
        // failed to compile. This test compiles the real output through the real ScriptLoader.
        var issues = types
            .Select(t => new DataQualityIssue
            {
                Type = t,
                Severity = IssueSeverity.High,
        ...
        var script = _generator.GenerateScript(issues, "CompileCheckScript");
        var scriptPath = Path.Combine(_tempDir, "CompileCheckScript.cs");
        await File.WriteAllTextAsync(scriptPath, script);

        var loader = new ScriptLoader(Path.Combine(_tempDir, ".cache"));
